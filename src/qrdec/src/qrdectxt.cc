      ...
      for(; j< sa_size && !err; j++)
      {
        ...
        for(k=0; k < qrdataj->nentries && !err; k++)
        {
          ...
          switch(entry->mode)
          {
            case QR_MODE_NUM:
            {
              Serial.printf("case QR_MODE_NUM\r\n");
            ...
            //case QR_MODE_BYTE:
            //  // 半角 小文字。 大文字に入れるとどう?
            //  Serial.printf("case QR_MODE_BYTE\r\n");
            case QR_MODE_ALNUM:
            {
              // 半角 大文字
              Serial.printf("case QR_MODE_ALNUM\r\n");
              char *p;
            ...
            case QR_MODE_BYTE:
              // 半角 小文字 上の 大文字に入れるとどう?
              Serial.printf("case QR_MODE_BYTE\r\n");
            case QR_MODE_KANJI:
            {
              Serial.printf("case QR_MODE_KANJI\r\n");
              in = (char *)entry->payload.data.buf;
              inleft = entry->payload.data.len;
              ...
              else
              {
                ...
                Serial.printf("QR_MODE_KANJI #10\n");
            ...
            case QR_MODE_ECI:
            {
              Serial.printf("case QR_MODE_ECI\r\n");
              const char *enc;
              char        buf[16];
              unsigned    cur_eci;
